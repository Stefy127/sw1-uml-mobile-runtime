...
class _DynamicFormPageState extends State<DynamicFormPage> {
  ...
  Future<void> _loadSchema() async { try { _schema ??= await _schemaService.load(); _normalizeRelations(); if (mounted) setState(() => _loading = false); } catch (e) { if (mounted) setState(() { _loading = false; _error = e.toString(); }); } }
  void _normalizeRelations() { for (final field in _fields.where((field) => field.relation)) { final value = _relations[field.name]; final target = _target(field.targetEntity); if (value is Map && target != null) { _relations[field.name] = value[target.idField]; } } }
  ...
  Future<void> _save() async {
    ...
    try { final id = widget.existingRecord?[widget.entity.idField]; if (widget.isEditing) { await _repository.update(widget.entity, id, body); } else { await _repository.create(widget.entity, body); } if (mounted) Navigator.of(context).pop(true); } catch (e) { if (mounted) setState(() => _error = e.toString()); } finally { if (mounted) setState(() => _saving = false); }
  }
  ...
  Widget _relationField(RuntimeField field) {
    final target = _target(field.targetEntity);
    if (target == null) {
      return const Text('Entidad relacionada no disponible.');
    }

    return FutureBuilder<List<Map<String, dynamic>>>(
      future: _repository.getAll(target),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const LinearProgressIndicator();
        }
        if (snapshot.hasError) {
          return Text('No se pudieron cargar las opciones de ${target.name}.');
        }
        ...
        return DropdownButtonFormField<dynamic>(
          ...
          items: records
              .map(
                (record) => DropdownMenuItem<dynamic>(
                  value: record[target.idField],
                  child: Text(
                    record[target.displayField]?.toString() ??
                        '${target.name} #${record[target.idField]}',
                  ),
                ),
              )
              .toList(),
          onChanged: (value) => setState(() {
            _relations[field.name] = value;
          }),
